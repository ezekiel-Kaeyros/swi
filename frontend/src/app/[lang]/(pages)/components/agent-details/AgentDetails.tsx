...
const AgentDetails = () => {
  ...
  return (
    <div className="z-20 w-[585px] h-screen bg-newPrimaryDark p-5 flex flex-col gap-10">
      <GenericPopupHeader
        textSize={'30px'}
        label={'Information'}
        closeCanceModal={hideAgentDetails}
        closeIconIcon={closeIconIcon}
      />
      <AgentDetailNavigation />
      {detailPageNavigation[0].status && <EmployeeBasicInfo />}
      {detailPageNavigation[1].status && <EmployeeActivities />}
      {detailPageNavigation[2].status && <EmployeePerformance />}
    </div>
  );
};
