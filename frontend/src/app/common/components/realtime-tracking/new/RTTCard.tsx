...
const RTTCard = ({ routeObj }: any) => {
  ...
  return (
    <div className="bg-bgColorDark rounded-xl p-4 flex flex-col gap-5 ">
      ...
      <DoubleConnectedChip
        textColor="text-activeTextColor"
        contentChipOne={routeObj?.elapsedTime}
        contentChipTwo={routeObj?.remainingTime}
        bgColor1="bg-activeBgColor"
        bgColor2={'bg-greyBg'}
      />
      ...
      <div className="flex flex-row gap-1 items-center justify-between">
        <div>
          <Link
            className="font-bold bg-settingViewBottomBorderColor rounded-full p-3 py-2 flex flex-row items-center gap-2"
            href={`/realtime-tracking/profile-view/${routeObj?.id}`}
          >
            <EyeIcon height="20" width="20" color="none" />
            <span className="text-[12px]">Track Route</span>
          </Link>
        </div>
        <div>
